    ...
    (do
      ...
      {:status 200
       ...
       :body (format
              ...
              (user/login-federated :destination "/user/" :fedIdentity "gmail.com")
              (user/login-federated :destination "/user/" :fedIdentity "myopenid.com"))
       }))
  ...
  (GET "/user/" [arg]
    (let [r (resp/response ;; construct ring response map
             (page/html5   ;; construct html content
              head
              [:body
               [:h1 "GAE User Service Tests"]
               environ
               [:h2 "Services"]
               [:ul
                [:li [:a {:href "/_ah/login_required"} "Login"]]
                [:li [:a {:href (user/logout-url :destination "/user/")} "Logout"]]
                 ]]))
