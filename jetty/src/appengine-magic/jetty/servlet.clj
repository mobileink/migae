...
(ns appengine-magic.jetty.servlet
...
;; from ring/util/servlet.clj
(defn make-service-method
;;   "Turns a handler into a function that takes the same arguments and has the
;;   same return value as the service method in the HttpServlet class."
;; in ae-m was: (defn make-servlet-service-method [ring-handler]
  (fn [^HttpServlet servlet,
       ^HttpServletRequest request,
       ^HttpServletResponse response]
    (let [response-map
          (doall (ring-handler (make-request-map servlet request response)))]
...
;; called from server/start to create servlet
;; (defn servlet [ring-handler]
;; "Create a servlet from a Ring handler.."
(defn make-servlet [ring-handler]
  (proxy [HttpServlet] []
    (service [^HttpServletRequest request, ^HttpServletResponse response]
      ((make-service-method ring-handler) this request response))))
